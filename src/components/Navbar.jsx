...
// const navItems = [
//   { label: "Get Admission Info", to: "/get-admission", highlight: true },
//   { label: "Home", to: "/" },
...
const Navbar = () => {
  ...
  return (
    <header className="sticky top-0 z-50 w-full px-4 py-4 sm:px-8 lg:px-16">
      {/* blink animation for highlighted nav item (Get Admission Info) */}
      <style>{`
        ...
        .blink-admission {
          animation: blinkRedGlow 1.1s ease-in-out infinite;
        }
      `}</style>
                ...
                <span
                  className={`relative inline-flex items-center rounded-full px-4 py-2 text-sm font-medium transition-all duration-300 ${
                    isActive
                      ? "bg-white text-black shadow-sm"
                      : "text-black/70 hover:bg-white/50 hover:text-black"
                  } ${item.highlight ? "blink-admission font-semibold" : ""}`}
                >
                  {item.label}
                </span>
              )}
